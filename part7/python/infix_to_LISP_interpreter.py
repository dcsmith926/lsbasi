...
class InfixToLISPInterpreter(NodeVisitor):
    
    def __init__(self, parser):
        self.parser = parser
    
    def visit_BinOp(self, node):
        return '(%s %s %s)' % (node.op.value, self.visit(node.left), self.visit(node.right))
...
def main():
    while True:
        ...
        lexer = Lexer(text)
        parser = Parser(lexer)
        interpreter = InfixToLISPInterpreter(parser)
